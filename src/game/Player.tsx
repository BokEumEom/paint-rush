...
export function Player() {
  ...
  useEffect(() => {
    const onDown = (event: KeyboardEvent) => {
      ...
      if (!rb || useGame.getState().phase !== 'wave' || document.pointerLockElement == null) return
  ...
  useFrame((state, delta) => {
    ...
    const bob = phase === 'wave' && document.pointerLockElement && speed > 1 ? Math.sin(state.clock.elapsedTime * 11) * 0.018 : 0
